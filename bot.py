...
async def check_subscriptions():
    ...
    for user_id, username, tariff, end_date_str, status in users:
        ...
        if status == 'active' and days_before_end <= 1:
            ...
            end_date = datetime.fromisoformat(end_date_str)
            grace_end = end_date + timedelta(days=2)

            today = now.date()
            grace_end_date = grace_end.date()

            days_left_in_grace = (grace_end_date - today).days

            if days_left_in_grace == 1:
                # сьогодні — останній день grace
                await bot.send_message(user_id,
                                       f"Залишився 1 день grace-періоду!\n"
                                       f"Підписка закінчується завтра {grace_end.strftime('%d.%m.%Y')}.\n"
